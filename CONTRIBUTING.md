...
| `src/settings-model.ts` | The settings shape, their defaults, and the saved-data merge rule (import-free, plain-Node testable) |
...
- Run `npm run smoke-test` - `bundle.ts`, `trust.ts`, `edits.ts` and `settings-model.ts` must pass their fixture checks (no Obsidian install needed for this one; it runs under plain Node against `scripts/fixtures/curation-bundle/`).
- **`npm run smoke-test` only covers the pure modules.** Anything that needs the Obsidian `App` - a vault scan via `metadataCache`, the review session's writes through `processFrontMatter`/`vault.modify`, the mtime guard on "corrected", split-leaf source opening, the settings tab - has no automated test (there's no headless Obsidian to run one in) and must be checked by hand in a real vault first. That boundary is why logic worth testing is moved *out* of `main.ts` rather than tested through it: `settings-model.ts` exists for exactly that reason.
