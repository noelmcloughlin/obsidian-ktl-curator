...
KTL Curator runs inside Obsidian, on the vault you open it in, and nowhere else.
...
- The sidecar templates' design, which is `knowledge-trust-ladder`'s. `knowledge-registrar.yaml`, `knowledge-release.yaml` and the files under `.lokf/scripts/` and `.lokf/m365/` are copies of its `skills/ktl-sidecar/templates/` and move only when copied again from there: from the release `TRUST_LADDER_SKILLS_REF` names, or from its `main` when a fix should not wait for the next tag; the pin then follows at that release. [CONTRIBUTING.md](CONTRIBUTING.md#before-opening-a-pull-request) says what moves with the pin. To confirm a copy, `diff` it against the templates at the copied ref; `knowledge-preflight.sh` also reports drift on its `copies` line where a sidecar is installed to compare against.
