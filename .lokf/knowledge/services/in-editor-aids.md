---
type: Service
id: https://lokf-curator.example/knowledge/services/in-editor-aids
...
- **Frontmatter autocomplete** - `src/suggest.ts`'s `LokfCuratorSuggest` (an `EditorSuggest`) offers values, never key names, inside a concept's frontmatter; the pure, Node-tested `src/suggest-context.ts` (`detectSuggestContext`, `withinFrontmatter`) decides what kind of value is being typed (actor / status / date) so the Obsidian-facing half stays thin.
