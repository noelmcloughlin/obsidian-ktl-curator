...
- Resolving configured bundle roots (`bundle.ts`'s `normalizeBundleRoots`/`resolveBundleRoot`; with none configured, `detectedRoot()` recognises a top-level `knowledge_bundle/index.md` in a vault whose root `index.md` has no LOKF header, via the pure `autoBundleRoot`) and re-scanning on `metadataCache` "changed" and `vault` "delete"/"rename" events, debounced 300ms. An absent root is explained only after checking Obsidian's live index, so a dot-folder root another plugin exposes is read like any other.
