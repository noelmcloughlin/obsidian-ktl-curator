...
* **Future-dated times corrected.** 3 `process:ktl-librarian` times were later than the commit that recorded them (round placeholders written as UTC). Each now carries that commit's time, the latest moment git attests, so the bundle keeps the sidecar's new conventions rule 11.
...
* **The docent records a reader's gap without reading the file it goes in.**
  ...
  never opens the file. The script comes from the `v0.23.0` templates, the
  release that ships it, and the pin moved to that tag with it.
...
* **The scheduled librarian installs its skill from a tag that has it.** The
  ...
  moves to `v0.23.0`, the release that also ships `knowledge-feedback.sh`. The
  scripts under `.lokf/scripts/` and `knowledge-registrar.yaml` all match its
  templates.
...
* **Reference audit moved onto the toolkit's own flag**: `just lokf-check-refs`
  ...
  step is gone and `--check-refs` rides on its validate step. That change
  has not reached the skills template yet, so this copy is ahead of it and
  the preflight's `copies` line reports the file as drifted until it does. That concept
