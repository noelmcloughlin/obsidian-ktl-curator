# `.lokf/`: KTL Curator's machine-readable knowledge base

This is a small **sidecar** that captures KTL Curator's own knowledge (its services, references, glossary, policies, playbooks, and explanation) as plain Markdown files that are **also a queryable knowledge graph**. It does not touch the plugin build; it is independent tooling you can run on its own.
...
|   |-- services/         # the plugin, trust engine, edits engine, side panel, settings tab, in-editor aids
|   |-- references/       # LOKF spec, OKF spec, lokf toolkit, Obsidian plugin guidelines, the curator skill's trust fields and review session
|   |-- glossary/         # OKF, LOKF, Diátaxis genre
|   |-- policies/         # no telemetry
|   |-- explanation/      # why KTL Curator exists
|   |-- playbooks/        # knowledge sources, releasing, contributing, the registrar gate
