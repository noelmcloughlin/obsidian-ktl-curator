...
// resolution, and the LOKF class check. Carried over from ktl-registrar's
...
// KTL Registrar) instead of a hard-coded list that silently drifts.
 ...
 *  from one a person customised - the same rule KTL Registrar applies. */
 ...
 *  doorway `ktl-sidecar` lays down as a link onto `.lokf/knowledge`, or the
 ...
 *  anyway. Kept identical to KTL Registrar's. */
...
export function isReserved(path: string): "index" | "log" | "diataxis" | null {
  ...
  // A Diátaxis map generated by KTL Registrar's affordances - not a concept to
