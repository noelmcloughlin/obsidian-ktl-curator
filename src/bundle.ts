...
// resolution, and the LOKF class check. Carried over from lokf-enforcer's
// validator.ts: multi-bundle-root path algebra and the LOKF class/relation
// vocabulary are shared plumbing, not enforcer rules.
//
...
// LOKF Enforcer) instead of a hard-coded list that silently drifts.
...
// Enforcer's, it is the pre-schema baseline, so schema-era additions (Role, and
...
export function isReserved(path: string): "index" | "log" | "diataxis" | null {
  ...
  // A Diátaxis map generated by LOKF Enforcer's affordances - not a concept to
